      ...
      <Section>
          ...
          {contactMethods.map((method, index) => (
            <Card key={method.title} className="group hover:shadow-lg transition-all duration-300 text-center animate-fade-in-up hover-lift" style={{ animationDelay: `${400 + index * 100}ms` }}>
              <div className="flex flex-col items-center p-4 md:p-6">
                <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-primary/10 flex items-center justify-center mb-3 md:mb-4 group-hover:bg-primary/20 transition-colors duration-300 ${method.color} hover-scale smooth-transition`}>
                  {method.icon}
                </div>
                <h3 className="text-lg sm:text-xl font-semibold text-text mb-2">{method.title}</h3>
                <p className="text-xs sm:text-sm text-text/70 mb-3 md:mb-4">{method.description}</p>
                <Link
                  href={method.href}
        ...
        <Card className="mb-8 md:mb-12 animate-fade-in-up animate-delay-700 hover-lift">
          <div className="text-center p-4 md:p-6">
            <h2 className="text-xl sm:text-2xl font-bold text-text mb-4 md:mb-6">Current Availability</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
              {availability.map((item, index) => (
                <div key={index} className="flex items-center gap-2 md:gap-3 p-2 md:p-3 bg-surface/30 rounded-lg hover-scale smooth-transition animate-fade-in-up" style={{ animationDelay: `${800 + index * 100}ms` }}>
                  ...
                  <span className="text-text/80 text-xs sm:text-sm">{item}</span>
                </div>
              ))}
            </div>
          </div>
